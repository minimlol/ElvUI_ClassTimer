...
if ( not P.unitframe.units.player ) then return; end
...
local MASTER_FONT = { LSM:Fetch("font", E.db.general.font), 12, "OUTLINE" };
local STACKS_FONT = { LSM:Fetch("font", E.db.general.font), 11, "OUTLINE" };
...
local CLASS_FILTERS = {
		...
		WARLOCK = {
			target = {
				...
				CreateSpellEntry( 146739 ), -- Corruption (Demo 5.4 PTR)
...
do
	...
	CreateAuraBarFrame = function( dataSource, parent )
		...
		background:SetTexture( LSM:Fetch('statusbar', E.private.general.normTex) );
		background:SetPoint( "TOPLEFT", result, "TOPLEFT", 0, 1 );
...
local playerFrame = CreateAuraBarFrame( playerDataSource, ElvUF_Player );
playerFrame:SetHiddenHeight( -yOffset );
playerFrame:SetPoint( "BOTTOMLEFT", ElvUF_Player, "TOPLEFT", 0, yOffset );
playerFrame:SetPoint( "BOTTOMRIGHT", ElvUF_Player, "TOPRIGHT", 0, yOffset );

playerFrame:Show();

local trinketFrame = CreateAuraBarFrame( targetDataSource, ElvUF_Player );
...
local targetFrame = CreateAuraBarFrame( trinketDataSource, ElvUF_Player );
